            ...
            <form action="{{ route('business.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <flux:input label="Nombre del Negocio" name="business_name" :value="old('business_name')"
                            required placeholder="Ej: Lavandería Express" :error="$errors->first('business_name')">
                            <x-slot:iconTrailing>
                                <x-heroicon-o-building-storefront class="w-5 h-5" />
                            </x-slot:iconTrailing>
                        </flux:input>

                        <flux:input label="Razón Social" name="legal_name" :value="old('legal_name')" required
                            placeholder="Ej: Lavandería Express S.A. de C.V." :error="$errors->first('legal_name')">
                            <x-slot:iconTrailing>
                                <x-heroicon-o-document-text class="w-5 h-5" />
                            </x-slot:iconTrailing>
                        </flux:input>

                        <flux:input label="RFC / Identificador Fiscal" name="tax_id" :value="old('tax_id')" required
                            placeholder="Ej: LEX850614HF3" :error="$errors->first('tax_id')">
                            <x-slot:iconTrailing>
                    ...
                    <div class="mt-4">
                        <flux:textarea label="Descripción del Negocio" name="description" rows="3"
                            placeholder="Describe brevemente los servicios que ofrece tu negocio..."
                            :error="$errors->first('description')">{{ old('description') }}</flux:textarea>
                    </div>
                </div>
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <flux:input label="Teléfono" name="phone" type="tel" :value="old('phone')" required
                            placeholder="+52 123 456 7890" :error="$errors->first('phone')">
                            <x-slot:iconTrailing>
                                <x-heroicon-o-phone class="w-5 h-5" />
                            </x-slot:iconTrailing>
                        </flux:input>

                        <flux:input label="Email (opcional)" name="email" type="email" :value="old('email')"
                            placeholder="contacto@negocio.com" :error="$errors->first('email')">
                            <x-slot:iconTrailing>
                        ...
                        <div class="md:col-span-2">
                            <flux:input label="Sitio Web (opcional)" name="website" type="url"
                                :value="old('website')" placeholder="https://www.sunegocio.com"
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <flux:input label="Dirección" name="address" :value="old('address')" required
                            placeholder="Calle, número, colonia" :error="$errors->first('address')">
                            <x-slot:iconTrailing>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <flux:input label="Ciudad" name="city" :value="old('city')" required
                                placeholder="Ciudad" :error="$errors->first('city')" />

                            <flux:input label="Estado" name="state" :value="old('state')" required
                                placeholder="Estado" :error="$errors->first('state')" />

                            <flux:input label="Código Postal" name="postal_code" :value="old('postal_code')" required
                                placeholder="12345" :error="$errors->first('postal_code')" />
                        </div>

                        <flux:input label="País" name="country" :value="old('country', 'México')" required
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <flux:input label="Latitud" name="latitude" type="number" step="any"
                            :value="old('latitude')" placeholder="19.432608" :error="$errors->first('latitude')" />

                        <flux:input label="Longitud" name="longitude" type="number" step="any"
                            :value="old('longitude')" placeholder="-99.133209" :error="$errors->first('longitude')" />
                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <flux:input label="Período de entrega (minutos)" name="delivery_period_minutes"
                            type="number" min="5" :value="old('delivery_period_minutes', 30)" required
                        ...
                        {{-- ✅ CAMBIAR ESTE BLOQUE (LÍNEA 229-234) --}}
                        <div class="flex items-center p-4 bg-gray-50 rounded-lg">
                            {{-- Hidden input para asegurar valor --}}
                            ...
                            <label for="can_be_rated" class="ml-3 text-sm font-medium text-gray-700">
